...
def posts_type(request, tid):
    posts_list_datas = Posts.objects.filter(b_type__id=tid).filter(status=1)
...
def posts_time(request, year, month):
    posts_list_datas = Posts.objects.filter(publish_time__year=year, publish_time__month=month).filter(status=1)
    return_value = get_posts_list_common_data(request, posts_list_datas)
